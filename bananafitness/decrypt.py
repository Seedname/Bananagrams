...
def evolve_keyspace(keyspace: dict, alphabet: str, single_letter_features: dict, current_features: list[dict], feature_counts: list[int],
    ...
    for i in range(len(keyspace)):
        ...
        key = keyspace[i]
        evolved_key, score = fitness.evolve_key(single_letter_features, current_features, feature_counts, message, alphabet, key=key)
...
def main() -> None:
    ...
    for word in message:
        ...
        decrypting_key = evolve_keyspace(possible_keys, alphabet, features["1"], current_features, feature_counts, message)
        encrypting_key = bananagrams.invert_key(decrypting_key, alphabet)
        print(f'{decrypting_key = }')
        print(f'{encrypting_key = }')
        bananagrams.decrypt(decrypting_key, '../encrypt/message.txt', '../bananafitness/correct.txt', alphabet)
